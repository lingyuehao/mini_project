...
class TestPandasUnit:
    def test_pd_loaded(self):
        ...
        need = {"Purchase_Amount","Gender","Education_Level",
                "Device_Used_for_Shopping","Payment_Method",
                "Customer_Satisfaction","Customer_ID"}
        assert isinstance(W.df_pd, pd.DataFrame) and len(W.df_pd) > 0
        assert need.issubset(W.df_pd.columns)

    def test_pd_purchase_amount_numeric_and_nonneg(self):
        """Pandas: Purchase_Amount is numeric and non-negative after cleaning."""
        s = W.df_pd["Purchase_Amount"]
    ...
    def test_pd_case1_shape_and_rows(self):
        ...
        valid = W.df_pd.dropna(subset=["Purchase_Amount"])
    ...
    def test_pd_edge_empty_group_ok(self):
        ...
        empty = W.df_pd[W.df_pd["Device_Used_for_Shopping"] == "__NO_SUCH_DEVICE__"]
        out = (empty.groupby("Payment_Method")
...
class TestPolarsUnit:
    def test_pl_loaded(self):
        ...
        need = {"Purchase_Amount","Gender","Education_Level",
                "Device_Used_for_Shopping","Payment_Method",
                "Customer_Satisfaction","Customer_ID"}
        assert isinstance(W.df_pl, pl.DataFrame) and W.df_pl.height > 0
        assert need.issubset(set(W.df_pl.columns))

    def test_pl_purchase_amount_float_and_nonneg(self):
        """Polars: Purchase_Amount is float dtype and non-negative when present."""
        assert W.df_pl.schema["Purchase_Amount"] in (pl.Float64, pl.Float32)
        s = W.df_pl.get_column("Purchase_Amount")
    ...
    def test_pl_case2_cols_and_types(self):
        ...
        b = W.case2_pl
        assert {"Payment_Method","avg_satisfaction","purchase_count"}.issubset(set(b.columns))
        int_ok = (pl.Int8, pl.Int16, pl.Int32, pl.Int64, pl.UInt8, pl.UInt16, pl.UInt32, pl.UInt64)
    ...
    def test_pl_edge_empty_group_ok(self):
        ...
        empty = W.df_pl.filter(pl.col("Device_Used_for_Shopping") == "__NO_SUCH_DEVICE__")
        out = (empty.group_by("Payment_Method")
               .agg([pl.col("Customer_Satisfaction").mean().alias("avg_satisfaction"),
...
class TestSystem:
    def test_case1_pd_vs_pl_agree(self):
        ...
        pl_df = (W.gender_edu_purchase_pl
                 .with_columns(
                     (pl.col("Gender").cast(pl.Utf8) + pl.lit("|") + pl.col("Education_Level").cast(pl.Utf8)).alias("key")
                 )
                 .select(["key","mean"]))
        b = dict(zip(pl_df.get_column("key").to_list(),
                     pl_df.get_column("mean").to_list()))
        assert set(a) == set(b)
        for k in a:
    ...
    def test_case2_pd_vs_pl_counts_agree(self):
        ...
        a = dict(zip(W.case2_pd.index.tolist(),
                     W.case2_pd["purchase_count"].tolist()))
        pl_keys = W.case2_pl.get_column("Payment_Method").to_list()
        pl_vals = W.case2_pl.get_column("purchase_count").to_list()
    ...
    def test_case2_index_matches_unique_methods(self):
        ...
        subset = W.df_pd[W.df_pd["Device_Used_for_Shopping"] == "Smartphone"]
        assert set(W.case2_pd.index) == set(subset["Payment_Method"].unique())
    ...
    def test_ml_table_numeric_and_no_nan(self):
        ...
        assert not W.df_ml[cols].isna().any().any()
        for c in W.features:
            assert pd.api.types.is_numeric_dtype(W.df_ml[c]) or pd.api.types.is_bool_dtype(W.df_ml[c])
